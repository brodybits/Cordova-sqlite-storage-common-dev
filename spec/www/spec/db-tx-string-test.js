  ...
  for (var i=0; i<scenarioCount; ++i) {
    ...
    describe(scenarioList[i] + ': tx string test(s)', function() {
      ...
      describe(suiteName + 'basic string binding/manipulation tests', function() {
        ...
        it(suiteName + "Inline emoji HEX test: SELECT LOWER(X'41F09F9883') [A\uD83D\uDE03] [\\u1F603 SMILING FACE (MOUTH OPEN)]", function(done) {
          if (isAndroid && !isWebSql && !isImpl2) pending('BROKEN for Android (default sqlite-connector version)');
          ...
          db.transaction(function(tx) {
            ...
            tx.executeSql("SELECT LOWER(X'41F09F9883') AS lowertext", [], function(tx, res) {
        ...
        it(suiteName + 'emoji parameter value test: "@\uD83D\uDE03!" [\\u1F603 SMILING FACE (MOUTH OPEN)]', function(done) {
          if (isAndroid && !isWebSql && !isImpl2) pending('BROKEN for Android (default sqlite-connector version)');
