    ...
    describe(scenarioList[i] + ': tx string test(s)', function() {
      ...
      describe(suiteName + 'basic string binding/manipulation tests', function() {
        ...
        it(suiteName + 'String encoding test with UNICODE \\u0000', function (done) {
          ...
          if (isAndroid && !isWebSql && !isImpl2) pending('BROKEN for Android (default sqlite-connector version)'); // [FUTURE TBD (documented)]
        ...
        // and Windows (SQLite3-WinRT)/(XX) versions.
      ...
      describe(suiteName + 'BLOB string test(s)', function() {
        ...
        it(suiteName + "SELECT LOWER(X'40414243') [BLOB string manipulation test]", function(done) {
          var db = openDatabase("BLOB-string-manipulation-test.db", "1.0", "Demo", DEFAULT_SIZE);

          db.transaction(function(tx) {

            tx.executeSql("SELECT LOWER(X'40414243') AS lowertext", [], function(ignored, rs) {
              expect(rs).toBeDefined();
              expect(rs.rows).toBeDefined();
              expect(rs.rows.length).toBe(1);
              expect(rs.rows.item(0).lowertext).toBe('@abc');
