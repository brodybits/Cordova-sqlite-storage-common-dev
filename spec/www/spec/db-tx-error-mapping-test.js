  ...
  for (var i=0; i<scenarioCount; ++i) {
    ...
    describe(scenarioList[i] + ': db tx error mapping test(s)', function() {
      ...
      describe(scenarioList[i] + ': basic tx error mapping test(s)', function() {
        ...
        it(suiteName + 'INSERT missing column (incorrect error code & inconsistent message on Windows/WP8)', function(done) {
          ...
          db.transaction(function(tx) {
            ...
            // Android/iOS WebKit Web SQL and Android implementation 2
            // (using built-in AOSP database classes) report 0 (UNKNOWN_ERR)
            // iOS plugin & default Android implementation report 5 (SYNTAX_ERR)
            if (isWindows)
              expect(error.code).toBe(-1);
            else if (isWebSql || (isAndroid && isImpl2))
              expect(error.code).toBe(0);
        ...
        it(suiteName + 'INSERT wrong column name (incorrect error code & inconsistent message on Windows/WP8)', function(done) {
          ...
          db.transaction(function(tx) {
            ...
            tx.executeSql('INSERT INTO test_table (wrong_column) VALUES (?)', ['abcdef'], function(tx) {
              ...
              // General case for Android/iOS (WebKit Web SQL & plugin)
              // [Plugin BROKEN (INCONSISTENT) on Windows]
              if (!isWindows)
      ...
      describe(suiteName + 'invalid parameter error code/message tests', function() {
        ...
        it(suiteName + 'Inline US-ASCII String manipulation test with arguments array=false', function(done) {
        ...
        it(suiteName + 'Inline US-ASCII String manipulation test with arguments array=true', function(done) {
          var db = openDatabase("Inline-US-ASCII-string-test-with-arguments-array-equals-true.db", "1.0", "Demo", DEFAULT_SIZE);

          db.transaction(function(tx) {

            try {
              tx.executeSql("SELECT UPPER('Some US-ASCII text') AS uppertext", true, function(ignored, res) {
                // ACTUAL BEHAVIOR for plugin (Android/iOS/Windows):
                if (isWebSql) expect('RESULT NOT EXPECTED for Web SQL').toBe('--');
        ...
        it(suiteName + 'Inline US-ASCII String manipulation test with arguments array="string-value"', function(done) {
      ...
      describe(suiteName + 'other error code/message mapping test(s)', function() {
        ...
        it(suiteName + 'transaction.executeSql syntax error with no SQL error handler [Windows error code/messages BROKEN/INCONSISTENT]', function(done) {
          if (isWP8) pending('SKIP for WP(8)'); // FUTURE TBD
        ...
        it(suiteName + 'transaction.executeSql constraint violation with no SQL error handler [Windows error code/messages BROKEN/INCONSISTENT]', function(done) {
          if (isWP8) pending('SKIP for WP(8)'); // FUTURE TBD
          ...
          db.transaction(function(tx) {
            ...
            // Extra SQLite info from iOS plugin:
