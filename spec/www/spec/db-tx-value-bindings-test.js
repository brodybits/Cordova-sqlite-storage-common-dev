      ...
      describe(suiteName + 'transaction column value insertion test(s)', function() {
        ...
        it(suiteName + 'INSERT TEXT string with emoji [\\u1F603 SMILING FACE (MOUTH OPEN)], SELECT the data, and check' +
           ((isWebSql && (isWindows || isWP8 || (isAndroid && !isImpl2))) ?
            ' [BROKEN: INCORRECT SELECT HEX(data) result on Android-sqlite-connector/Windows/WP8]' : ''), function(done) {
        ...
        it(suiteName + "INSERT inline BLOB value (X'40414243') and check stored data [SELECT BLOB ISSUE with androidDatabaseImplementation: 2 & Windows]", function(done) {
