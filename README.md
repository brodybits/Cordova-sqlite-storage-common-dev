...
- Windows version uses the performant C++ [doo / SQLite3-WinRT](https://github.com/doo/SQLite3-WinRT) component and has the following limitations:
  ...
  - Missing error code and inconsistent error message in error callbacks
...
- Certain errors such as CREATE VIRTUAL TABLE USING bogus module are reported with error code 5 (SYNTAX_ERR) on Android and iOS. This happens to be the case for WebKit Web SQL (Android/iOS).
- SQL errors are reported with error code 0 on Windows. In certain cases SQL errors are reported with error code 0 on Windows in case the built-in Android database used (using the `androidDatabaseImplementation: 2` setting in `window.sqlitePlugin.openDatabase`).

#### XXX Some test/doc TODOs

XXX string chars both HEX(?) binding & inline 'manipulation'
XXX new evcore
XXX new alt

XXX test/doc alt parameter bindings ?NNN/:XX/$XX/@XX
XXX results with column name from SELECT statement

XXX test sql batch with invalid data

XXX doc WebKit Web SQL vs plugin: empty tx, invalid tx.executeSql parameters

XXX test/doc WebKit SQL vs plugin: more invalid tx, tx.executeSql args

- XXX TBD SQL errors reported with error code 0 Android impl2
- XXX double-check tested all possible SQL values including true/false, undefined, ...
- XXX test binding with parameters: null, undefined/missing, false, true, string, object, ... ...
- XXX test db SQL, parameters: valid, null, undefined/missing, true/false, string/object, ... ...
- XXX test sqlBatch with invalid arguments, strange/invalid members in the array
- XXX SQLTransaction.executeSql with not enough parameters *not* rejected by plugin (does not cause data loss or break compliant programs, will not be changed to avoid breaking application code that may already be working with not enough parameters)
- XXX executeSql with too many parameters *not* rejected by plugin on iOS: #552

XXX test+doc insertId should throw [or be undefined for plugin] if no inserted rows

XXX doc multi-row insert insertId quirk ref: https://www.w3.org/TR/webdatabase/#database-query-results
XXX doc/double-check emoji issue Android-sqlite-connector

XXX test/doc INSERT from SELECT

XXX attempt to use BEGIN/COMMIT/ROLLBACK within SQL ref: https://www.w3.org/TR/webdatabase/#dom-sqlexception-code-syntax

### Other differences from WebKit Web SQL implementations
...
#### XXX Further test/doc TODOs

- XXX doc multi-row INSERT with parameters
- XXX doc BIG integer value bindings

XXX TODO test/doc multi row INSERT binding db sql/sql batchList
XXX TODO test error mapping with new errors from test
XXX TODO test error mapping when tx function, success cb, or error cb throws
XXX TODO test error mapping with db.sql & sqlBatch
...
- It is possible to request a SQL statement list such as "SELECT 1; SELECT 2" within a single SQL statement string, however the plugin will only execute the first statement and silently ignore the others ref: [litehelpers / Cordova-sqlite-storage#551](https://github.com/litehelpers/Cordova-sqlite-storage/issues/551)
- The Android version does not always handle four-byte UTF-8 characters emoji characters such as `\u1F603` (SMILING FACE, MOUTH OPEN) correctly. It is sometimes possible to store and retrieve such characters but certain operations hex conversions do not work properly with the default [Android-sqlite-connector](https://github.com/liteglue/Android-sqlite-connector) database implementation. This is not an issue in case the built-in Android database used (using the `androidDatabaseImplementation: 2` setting in `window.sqlitePlugin.openDatabase`)
- INSERT statement that affects multiple rows (due to SELECT cause or using TRIGGER(s), for example) does not report proper rowsAffected on Android in case the built-in Android database used (using the `androidDatabaseImplementation: 2` setting in `window.sqlitePlugin.openDatabase`)
...
- UNICODE `\u0000` (same as `\0`) character not working in Android (default [Android-sqlite-connector](https://github.com/liteglue/Android-sqlite-connector) database implementation) or Windows
