...
- Windows version is in an alpha state:
  - XXX TEST (probably fixed): Windows 8.1/Windows Phone 8.1 build disabled due to certain build issues
  - XXX TEST (probably fixed): multiple updates with key scenario
...
- BLOB type is ~~currently~~ not supported _in this version branch (TBD BLOB read to be supported in cordova-sqlite-ext)_.
